 ...
 * @brief Basic image processing template
 */
...
bool imageProcessing::configure(yarp::os::ResourceFinder &rf) {
    /* Process all parameters from both command-line and .ini file */

    /* get the module name which will form the stem of all module port names */
    moduleName            = rf.check("name", 
                           Value("/imageProcessingEdge"), 
    ...
    */
    ...
    verticalOffset = rf.check("verticalOffset", 
                           Value(-1), 
                           "vertical offset rectification (double)").asFloat64();
    ...
    */
    ...
    rThread = new imageProcessingThread(robotName, configFile);
...
bool imageProcessing::interruptModule() {
    handlerPort.interrupt();
    return true;
}

bool imageProcessing::close() {
    handlerPort.close();
    /* stop the thread */
...
bool imageProcessing::respond(const Bottle& command, Bottle& reply) 
{
    bool ok = false;
...
bool imageProcessing::updateModule()
{
    return true;
}

double imageProcessing::getPeriod()
{
