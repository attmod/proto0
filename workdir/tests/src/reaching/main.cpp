...
public:
    bool configure(ResourceFinder &rf)
    {
        ...
        float left_fx  = static_cast<float>(cam_left_info->get(0).asFloat64());
        float left_cx  = static_cast<float>(cam_left_info->get(2).asFloat64());
        float left_fy  = static_cast<float>(cam_left_info->get(5).asFloat64());
        float left_cy  = static_cast<float>(cam_left_info->get(6).asFloat64());
        ...
        float right_fx = static_cast<float>(cam_right_info->get(0).asFloat64());
        float right_cx = static_cast<float>(cam_right_info->get(2).asFloat64());
        float right_fy = static_cast<float>(cam_right_info->get(5).asFloat64());
        float right_cy = static_cast<float>(cam_right_info->get(6).asFloat64());
...
//                estimates->setSubvector(6, estimates->subVector(6, 8) + vel_x);
                ...
                l_px_endeffector.addInt16(l_px[0]);
                l_px_endeffector.addInt16(l_px[1]);
                port_px_left_endeffector.write();

                Bottle& r_px_endeffector = port_px_right_endeffector.prepare();
                r_px_endeffector.clear();
                r_px_endeffector.addInt16(r_px[0]);
                r_px_endeffector.addInt16(r_px[1]);
    ...
    bool respond(const Bottle& command, Bottle& reply)
    {
        ...
        switch (cmd)
        {
            ...
            case VOCAB3('i','m','g'):
            {
                ...
                l_px_goal_.resize(2);
                l_px_goal_[0] = click_left->get(0).asFloat64();
                l_px_goal_[1] = click_left->get(1).asFloat64();

                r_px_goal_.resize(2);
                r_px_goal_[0] = click_right->get(0).asFloat64();
                r_px_goal_[1] = click_right->get(1).asFloat64();
                ...
                if (rep.size()>1)
                {
                    if (rep.get(0).asVocab32() == Vocab32::encode("ack"))
                    {
                        if (Bottle *idField = rep.get(1).asList())
                        {
                            ...
                            {
                                int id = idValues->get(0).asInt16();

                                cmd.clear();

                                cmd.addVocab32(Vocab32::encode("get"));
                                Bottle& content = cmd.addList();
                                Bottle& list_bid = content.addList();

                                list_bid.addString("id");
                                list_bid.addInt16(id);
                                ...
                                if (reply_prop.get(0).asVocab32() == Vocab32::encode("ack"))
                                {
                                    if (Bottle* propField = reply_prop.get(1).asList())
                                    {
                                        if (Bottle* position_2d = propField->find("position_2d_left").asList())
                                        {
                                            ...
                                            {
                                                l_px_goal_.resize(2);
                                                l_px_goal_[0] = position_2d->get(0).asFloat64() + (position_2d->get(2).asFloat64() - position_2d->get(0).asFloat64()) / 2;
                                                l_px_goal_[1] = position_2d->get(1).asFloat64() + (position_2d->get(3).asFloat64() - position_2d->get(1).asFloat64()) / 2;
                                                ...
                                                cmd.addInt16(l_px_goal_[0]);
                                                cmd.addInt16(l_px_goal_[1]);
                                                ...
                                                {
                                                    location_.resize(3);
                                                    location_[0] = reply_pos.get(0).asFloat64();
                                                    location_[1] = reply_pos.get(1).asFloat64();
                                                    location_[2] = reply_pos.get(2).asFloat64();

                                                    r_px_goal_.resize(2);
                                                    r_px_goal_[0] = reply_pos.get(3).asFloat64();
                                                    r_px_goal_[1] = reply_pos.get(4).asFloat64();
                                                }
                                                else
                                                {
